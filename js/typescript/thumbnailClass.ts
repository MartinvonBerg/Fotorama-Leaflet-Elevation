...
class ThumbnailSlider implements thumbnailClassInterface {
  ...
  constructor(number : number, options: optionsType ) {
    ...
    let thumbWidth : number = this.ele.parentElement.offsetWidth;
   ...
   */
   private updateCSS() : void {
    ...
    let h : number = 0.8 * parseInt(this.options.bar_min_height);
   ...
   */
   private mouseMoveHandler(e : any) : void {
    ...
    let dx : number = e.clientX - this.pos.x;
   ...
   */
   private mouseUpHandler(e : any) : void {
    ...
    let posXDelta : number = e.clientX - this.posXOld;
    
    if ( (Math.abs(posXDelta) < 5) ) {
      let thnumb : number = parseInt(e.composedPath()[1].id.replace('thumb','')) 
   ...
   */
   public setActiveThumb(number : number) : void {
    ...
    let xOffset : number = this.ele.getBoundingClientRect().left;

    if (this.thumbnails[number].getBoundingClientRect().x - xOffset < 10) { // to left
      let toLeft : number = this.thumbnails[number].getBoundingClientRect().x -xOffset;
   ...
   */
   private resizer = (e : any) : void => {
    ...
    let number : number = this.currentActive;
    let wrapperWidth : number = this.ele.parentElement.offsetWidth;  // thumb_wrapper
    let allImagesdWidth : number = this.sumAspectRatios * this.thumbnails[number].offsetHeight;
    if ( allImagesdWidth < 2) return;
    
    let parentWidth : number = this.ele.offsetWidth; // thumb_inner
    let eleWidth : number = this.thumbnails[number].offsetWidth;
    let offsetLeft : number = (parentWidth - eleWidth) / 2 // das ist das Ziel für den Offset.
    let distLeft : number = this.thumbnails[number].getBoundingClientRect().x;
