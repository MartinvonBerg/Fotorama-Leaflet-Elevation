function mainLogic (window, document, customDocument=null, win=null, vars=null) {
    //"use strict";
    // Verwende customDocument, wenn übergeben, andernfalls das normale document
    ...
    if (customDocument !== null) {
        console.log('customDocument gefunden! Aufruf im Editor mit: ', numberOfBoxes);
        if (vars !== null) pageVarsForJs = vars;
        if (win !== null) usrAgent = win.navigator.userAgent.toLowerCase();
    } 

    if ( numberOfBoxes > 0 ) {
        let isMobile = (/iphone|ipod|android|webos|ipad|iemobile|blackberry|mini|windows\sce|palm/i.test(usrAgent));
        ...
        for (let m = 0; m < numberOfBoxes; m++) {
            ...
            //------------- Slider part --------------------------------------
            hasFotorama = doc.querySelectorAll('[id^=mfotorama'+m+']').length === 1;
            hasSwiper = doc.querySelectorAll('[id^=swiper'+m+']').length === 1;
            hasMasonry1 = doc.querySelectorAll('[id^=minimasonry'+m+']').length === 1;

            let sliderSel = '';

            //------------- leaflet - elevation - chart part ---------------------------
            hasMap = doc.querySelectorAll('[id^=boxmap'+m+']').length === 1;
            hasChartJS = doc.querySelectorAll('[id^=chartjs-profile-container'+m+']').length === 1;
            hasLeafElev = doc.querySelectorAll('[id^=elevation-div'+m+']').length === 1; 

            // remove grid class if parent is a column.
            let el = doc.getElementById('multifotobox'+m);
            let parentClass = el.parentElement.className;
            if (parentClass.includes('column')) {
                el.classList.remove('mfoto_grid');
                el = doc.getElementById('boxmap'+m);
            ...
            if ( hasMap && (hasFotorama || hasSwiper) ) {
                ...
                doc.querySelector('#'+sliderSel+ m).addEventListener('sliderchange', function waschanged(e) {
                ...
                // update markers on the map if the active image changes
                doc.querySelector('#'+sliderSel+ m).addEventListener('sliderload', function wasloaded(e) {
                ...
                doc.querySelector('#boxmap'+ m).addEventListener('mapmarkerclick', function markerclicked(e) {
        ...
        function resizer(event) {
            ...
            if( isMobile && hasFotorama) {
                doc.querySelector('.fotorama__caption__wrapm, .fotorama__caption').style.display='none';
            }
            /*
            if( isMobile && hasSwiper) {
                const el = doc.querySelectorAll('.swiper-slide-title');
                el.forEach(element => {
            ...
            */           
            for (let m = 0; m < numberOfBoxes; m++) {
                ...
                if (typeof(allMaps[m]) === 'object') {
                    ...
                    let wmap = parseFloat( getComputedStyle( doc.querySelector('#map' + m), null).width.replace("px", ""));
                    let hmap = parseFloat( getComputedStyle( doc.querySelector('#map' + m), null).height.replace("px", ""));
