...
function fotorama_multi_styles( ) {
	...
	if ( $fotorama_elevation_options['min_width_css_grid_row_14'] > 0) {
		$stylestring_escaped  = '<style id="fotorama_multi_inline_css" type="text/css">';
		$stylestring_escaped  .= '@media screen and (min-width: 480px) { .mfoto_grid { display: grid;';
		$stylestring_escaped  .= ' grid-template-columns: repeat(auto-fit, minmax('. esc_attr($fotorama_elevation_options['min_width_css_grid_row_14']) .'px, 1fr)); grid-gap: 5px;} } </style>';  
		echo $stylestring_escaped;
	}
}
...
function enqueue_style_tag( $cssFile) {
	$stylestring_escaped  = '<link rel="stylesheet" href="'. esc_attr($cssFile) .'" media="print" onload="this.media=\'all\'">';
	echo $stylestring_escaped;
	do_action( 'qm/debug', $stylestring_escaped );
	return;
}
*/
...
/*
 function addLinkToHead() {
	...
	elseif ( $isMobile ) {
		...
		echo htmlspecialchars_decode( esc_html($link2) );
	} else {
 ...
 function fm_error_notice() {
    ...
    <div class="notice notice-warning is-dismissible">
		...
		foreach ($fm_act_pis as $conflict) {
			echo htmlspecialchars_decode( esc_html( '<strong>' . $conflict . '</strong><br>')); 
		}
		?>
