...
if ( is_admin() ) {
	...
	$fotorama_elevation_options = is_array(get_option('fm_leaflet_options')) ? get_option('fm_leaflet_options') : [];
	$fotorama_elevation_options['htaccess_Tile_Server_Is_OK'] = $fotorama_elevation->checkHtaccess() ? 'true' : 'false';
...
function showmulti($attr, $content = null)
{
	...
	\wp_enqueue_style('swiperCss', $plugin_path . 'js/swiperClass.min.css',[],'0.34.1','all');
	\wp_enqueue_style('swiperThumbsCss', $plugin_path . 'js/thumbnailClass.min.css',[],'0.34.1','all');
...
EOF;
	...
	if ( isset($charttype) && $charttype === 'chartjs') {
		wp_enqueue_script('fotorama_main_bundle',  $plugin_path . 'build/fm_chartjs/fm_main.js', ['jquery'], '0.34.1', true);
	} else {
		wp_enqueue_script('fotorama_main_bundle',  $plugin_path . 'build/fm_bundle/fm_main.js', ['jquery'], '0.34.1', true);
	}
